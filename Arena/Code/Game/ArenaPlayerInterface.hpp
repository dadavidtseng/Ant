...
struct ArenaTurnStateForPlayer
{
	...
	AgentReport agentReports[MAX_REPORTS_PER_PLAYER]; // only entries less than [numReports] are valid
	int numReports;

	// List of all agents (besides yours) within your current visibility
	ObservedAgent observedAgents[MAX_AGENTS_TOTAL]; // only entries less than [numObservedAgents] are valid
